...
int Show(char *name,int age,int height,char *m_sc,char *m_yz);  // 显示超女基本信息的成员函数体
...
int main()
{
  ...
  // Girl.Show();   // 调用对象的成员函数
  Show(Girl.m_name,Girl.m_age,Girl.m_height,Girl.m_sc,Girl.m_yz);
}
...
/*
int Show(char *name,int age,int height,char *sc,char *yz)  // 显示超女基本信息的成员函数体
{
  printf("姓名：%s，年龄：%d，身高：%d，"\
         "身材：%s，颜值：%s\n",\
